...
class CTM_Recovery
{
		...
		if($_GET["code"] == TRUE)
		{
			if($_GET["check"] == TRUE)
			{
				...
				else
				{
					...
					$Check_Link = CTM_MSSQL::getInstance()->NumQuery("SELECT * FROM dbo.CTM_WebRecovery WHERE HashCode={$Encode}");
					$Find_Link = CTM_MSSQL::getInstance()->FetchQuery("SELECT Status,Account,Mail,Time_,Expiration 
					FROM dbo.CTM_WebRecovery WHERE HashCode={$Encode}");
			...
			if($_GET["cmd"] == TRUE)
			{
				...
				else
				{
					...
					{
						CTM_MSSQL::getInstance()->Query("UPDATE dbo.CTM_WebRecovery SET Status=0x001F WHERE HashCode={$Encode}");
						if(USE_MD5 == 1)
						{
							CTM_MSSQL::getInstance()->Query("exec dbo.CTM_CryptPwd '".$Find_Link[1]."','".$Password."'");
						}
						else
						{
							CTM_MSSQL::getInstance()->Query("UPDATE ".MuAcc_DB.".dbo.MEMB_INFO SET memb__pwd='{$Password}' WHERE memb___id='".$Find_Link[1]."'");
						}
	...
	private function Prepare()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			$Mail = $_POST["Mail"];
			$Captcha = $_POST["Captcha"];
			$Check = CTM_MSSQL::getInstance()->NumQuery("SELECT mail_addr FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE mail_addr='{$Mail}'");
			$Find_Mail = CTM_MSSQL::getInstance()->FetchQuery("SELECT mail_chek FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE mail_addr='{$Mail}'");
			...
			else
			{
				...
				$Find_Account = CTM_MSSQL::getInstance()->FetchQuery("
				SELECT memb___id,memb_name,fpas_ques,fpas_answ FROM ".MuAcc_DB.".dbo.MEMB_INFO 
				WHERE mail_addr='{$Mail}'");
				CTM_MSSQL::getInstance()->Query("INSERT INTO dbo.CTM_WebRecovery (Account,Mail,HashCode,Time_,Expiration,Status) 
				...
				else
				{
					if(constant("Recovery_Pass") === TRUE)
					{
						...
						if(USE_MD5 == 1)
						{
							CTM_MSSQL::getInstance()->Query("exec dbo.CTM_CryptPwd '".$Find_Account[0]."','".$New_Password."'");
						}
						else
						{
							CTM_MSSQL::getInstance()->Query("UPDATE ".MuAcc_DB.".dbo.MEMB_INFO SET memb__pwd='{$New_Password}' WHERE memb___id='".$Find_Account[0]."'");
						}
					}
					unset($Link);
					unset($Message);
	...
	private function Check_Datas()
	{
		...
		if($_GET["cmd"] == "verify")
		{
			if($_GET["id"] == "code")
			{
				...
				$Check = CTM_MSSQL::getInstance()->NumQuery("SELECT * FROM dbo.".$GLOBALS["CTM"][14]." WHERE HashCode={$HashCode}");
