...
class CTM_Ajax
{
	...
	private function Panel($Account)
	{
		...
		{
			$Query = CTM_MSSQL::getInstance()->Query("SELECT * FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id='{$Account}'");
			$Staff = CTM_MSSQL::getInstance()->FetchQuery("SELECT type FROM CTM_WebStaff WHERE account='{$Account}'");
			$Type_Acc = CTM_MSSQL::getInstance()->FetchQuery("SELECT ".VIP_Column." FROM ".VIP_DB.".dbo.".VIP_Table." WHERE ".VIP_Login."='{$Account}'");
			$Golds = CTM_MSSQL::getInstance()->FetchQuery("SELECT ".GL_Column_1.",".GL_Column_2.",".GL_Column_3." FROM ".GL_DB.".dbo.".GL_Table." WHERE ".GL_Login."='{$Account}'");
			$Load_Acc = CTM_MSSQL::getInstance()->FetchArray($Query);
	...
	private function Login_Command()
	{
		...
		if($_GET["cmd"] == "login")
		{
			...
			$Check = CTM_MSSQL::getInstance()->FetchQuery("exec dbo.CTM_CheckLogin '".$Account."','".$Password."',".USE_MD5."");
			$Find = CTM_MSSQL::getInstance()->FetchQuery("SELECT bloc_code,mail_chek FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id='{$Account}'");
	...
	public function ServerList()
	{
		...
		else
		{
			...
			echo("<li>Total: <strong id=\"TotalServers\">".$CTM_ConnectServer->ServerCount(false, false)."</strong> <a href=\"javascript: void(EffectWeb);\" onClick=\"CTM_Load('?ajax=check&cmd=servers', 'ServerRefresh', 'GET');\"><img src=\"images/icons/refresh.png\" width=\"10\" height=\"10\" border=\"0\"></a></li>");
		}
	}
