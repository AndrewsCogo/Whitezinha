...
class CTM_Register
{
	...
	public function __construct()
	{
		...
		{
			if(CTM_General::getInstance()->Check_Logged(false) == TRUE)
			{
	...
	private function RegisterNow()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			# Define Check POST
			$Check[0] = CTM_Mssql::getInstance()->NumQuery("SELECT memb___id FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id=".($lower ? "LOWER" : NULL)."('{$Account}')");
			$Check[1] = CTM_Mssql::getInstance()->NumQuery("SELECT mail_addr FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE mail_addr='{$Mail}'");
			...
			if(constant("Register[Confirm_Mail]") === TRUE)
			{
				...
				CTM_Mssql::getInstance()->Query("INSERT INTO dbo.CTM_WebRegister (Account,Temp_Account,HashCode,Status) VALUES (
			...
			CTM_Mssql::getInstance()->Query("INSERT INTO ".MuAcc_DB.".dbo.MEMB_INFO (memb___id,memb__pwd,memb_name,sno__numb,post_code,addr_info,addr_deta,tel__numb,mail_addr,phon_numb,fpas_ques,fpas_answ,job__code,CTM_Date,CTM_Birth,CTM_Sex,appl_days,modi_days,out__days,true_days,mail_chek,bloc_code,ctl1_code) VALUES ('".$Define["Account"]."',".$this->Pwd_Query.",'{$Name}','{$PID}','s-n','11111','','{$Phone}','{$Mail}','','{$Question}','{$Answer}','1','{$Date}','{$Date_G}','{$Sex}','2003-11-23','2003-11-23','2003-11-23','2003-11-23','".$Define["Mail_Check"]."','0','1')");
			if (USE_MD5 == 1)
			{
				CTM_Mssql::getInstance()->Query("exec dbo.CTM_CryptPwd '".$Define["Account"]."','".$Password."'");
			}
			
			# Define Register VI_CURR_INFO
			if(VI_CURR_INFO == TRUE)
			{
				CTM_Mssql::getInstance()->Query("INSERT INTO ".MuAcc_DB.".dbo.VI_CURR_INFO (ends_days,chek_code,used_time,memb___id,memb_name,memb_guid,sno__numb,Bill_Section,Bill_value,Bill_Hour,Surplus_Point,Surplus_Minute,Increase_Days) VALUES ('2005','1',1234,'".$Account."','{$Name}',1,'7','6','3','6','6','2003-11-23 10:36:00','0')");
			}
			
			# Define VIP Table
			if(VIP_Table != "MEMB_INFO")
			{
				CTM_Mssql::getInstance()->Query("INSERT INTO ".VIP_DB.".dbo.".VIP_Table." (".VIP_Login.",".VIP_Column.",".VIP_Begin.",".VIP_Time.",".VIP_Credits.") VALUES ('".$Define["Account"]."',0,0,0,0)");
			}
			
			# Define Cash Table
			if(GL_Table != "MEMB_INFO")
			{
				CTM_Mssql::getInstance()->Query("INSERT INTO ".GL_DB.".dbo.".GL_Table." (".GL_Column_1.",".GL_Login.") VALUES (0,'".$Define["Account"]."')");
			}
			...
			if(constant("Register[VIP][Enable]") === TRUE)
			{
				...
				CTM_Mssql::getInstance()->Query("UPDATE ".VIP_DB.".dbo.".VIP_Table." SET ".VIP_Column."=".constant("Register[VIP][Type]").",".VIP_Begin."=".$VIP_Begin.",".VIP_Time."=".$VIP_Time.",".VIP_Credits."=".constant("Register[VIP][Time]")." WHERE ".VIP_Login."='".$Define["Account"]."'");

				$Bonus .= "&raquo; ".constant("Register[VIP][Time]")." dias de ".$VIP_Type.".<br />";
			}
			if(constant("Register[Cash][Enable]") === TRUE)
			{
				CTM_Mssql::getInstance()->Query("UPDATE ".GL_DB.".dbo.".GL_Table." SET ".constant("GL_Column_".constant("Register[Cash][Coin]"))."=".constant("GL_Column_".constant("Register[Cash][Coin]"))."+".constant("Register[Cash][Number]")." WHERE ".GL_Login."='".$Define["Account"]."'");
			...
			if(!empty($_COOKIE['NewFinal_Reference']))
			{
				...
				$query = CTM_Mssql::getInstance()->Query("SELECT Account FROM dbo.CTM_WebReference WHERE Id = '{$reference}'");
				$check = $this->NumRow($query);
				$fetch = $this->Fetch($query);
				
				if($check > 0)
				{
					$query = CTM_Mssql::getInstance()->Query("SELECT Account FROM dbo.CTM_WebReferenceData WHERE Reference = '{$reference}' AND (Account = '{$Account}' OR IPAddress = '{$IP}')");
					$check = $this->NumRow($query);
					
					if($check < 1)
					{
						CTM_Mssql::getInstance()->Query("UPDATE dbo.CTM_WebReference SET RegisterCount = RegisterCount + 1, Points = Points + ".NEWFINAL_REF_REGPOINTS." WHERE Id = {$reference}");
						CTM_Mssql::getInstance()->Query("INSERT INTO dbo.CTM_WebReferenceData (Reference,RefLogin,Account,IPAddress) VALUES ({$reference},'{$fetch[0]}','{$Account}','{$IP}')");
					}
				}
			}
	...
	private function Confirm_Register()
	{
		...
		if($_GET["code"] == TRUE)
		{
			...
			else
			{
				...
				$Check_Link = CTM_Mssql::getInstance()->NumQuery("SELECT * FROM dbo.CTM_WebRegister WHERE HashCode={$Encode}");
				$Find_Link = CTM_Mssql::getInstance()->FetchQuery("SELECT Status,Account,Temp_Account 
				FROM dbo.CTM_WebRegister WHERE HashCode={$Encode}");
				...
				{
					CTM_Mssql::getInstance()->Query("UPDATE dbo.CTM_WebRegister SET Status=0xF41E WHERE HashCode={$Encode}");
					CTM_Mssql::getInstance()->Query("UPDATE ".MuAcc_DB.".dbo.MEMB_INFO SET memb___id='".$Find_Link[1]."',mail_chek=1 WHERE memb___id='".$Find_Link[2]."'");
					CTM_Mssql::getInstance()->Query("UPDATE ".MuGen_DB.".dbo.warehouse SET AccountID='".$Find_Link[1]."' WHERE AccountID='".$Find_Link[2]."'");
					if(VIP_table != "MEMB_INFO")
					{
						CTM_Mssql::getInstance()->Query("UPDATE ".VIP_DB.".dbo.".VIP_Table." SET ".VIP_Login."='".$Find_Link[1]."' WHERE ".VIP_Login."='".$Find_Link[2]."'");
					}
					if(GL_Table != "MEMB_INFO")
					{
						CTM_Mssql::getInstance()->Query("UPDATE ".GL_DB.".dbo.".GL_Table." SET ".GL_Login."='".$Find_Link[1]."' WHERE ".GL_Login."='".$Find_Link[2]."'");
					}
	...
	private function VerifyDatas()
	{
		if($_GET["cmd"] == "verify")
		{
			...
			if($_GET["id"] == "code")
			{
				...
				$Check = CTM_Mssql::getInstance()->NumQuery("SELECT * FROM dbo.".$GLOBALS["CTM"][18]." WHERE HashCode={$HashCode}");
			...
			if($_GET["id"] == "login")
			{
				$lower = (boolean)constant("Register[ForceLower]");
				$Acc = $_GET["account"];
				$Check = CTM_Mssql::getInstance()->NumQuery("SELECT memb___id FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id=".($lower ? "LOWER" : NULL)."('{$Acc}')");
			...
			if($_GET["id"] == "mail")
			{
				...
				$Mail_Addr = $_GET["mail"];
				$Check = CTM_Mssql::getInstance()->NumQuery("SELECT mail_addr FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE mail_addr='{$Mail_Addr}'");
